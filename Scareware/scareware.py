...
def agregar_a_inicio():
    ruta_python = sys.executable  # Ruta del ejecutable de Python
    ruta_script = os.path.abspath(__file__)  # Ruta absoluta del script

    clave = r"Software\Microsoft\Windows\CurrentVersion\Run"
    nombre_valor = "BloqueoPC"  # Nombre de la clave en el registro

    try:
        with reg.OpenKey(reg.HKEY_CURRENT_USER, clave, 0, reg.KEY_SET_VALUE) as key:
            reg.SetValueEx(key, nombre_valor, 0, reg.REG_SZ, f'"{ruta_python}" "{ruta_script}"')
        print("Script agregado al inicio correctamente.")
